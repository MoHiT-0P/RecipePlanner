...
import { cleanIngredient } from '../utils/helpers.js'; // Import the new helper
...
export default function Admin() {
  ...
  const renderView = () => {
    switch (view) {
      case 'addRecipe':
        return <AddRecipeForm onAddSuccess={handleImportSuccess} />;
      case 'manageUsers':
        return <ManageUsers />;
      case 'importRecipes':
        return <ImportRecipes onImportSuccess={handleImportSuccess} />;
      case 'manageRecipes':
      default:
  ...
  // ... (Rest of Admin component is the same) ...
  return (
...
// --- Import Recipes Component (UPDATED) ---
const ImportRecipes = ({ onImportSuccess }) => {
    // ... (state and searchRecipes function are the same) ...
    ...
    const searchRecipes = async (e) => {
        e.preventDefault(); if (!query) return; setLoading(true); setError(''); setResults([]);
        try {
            const response = await fetch(`https://api.spoonacular.com/recipes/complexSearch?query=${query}&number=10`, { headers: { 'x-api-key': SPOONACULAR_API_KEY } });
            if (!response.ok) { const errorData = await response.json(); throw new Error(errorData.message || 'Failed to fetch recipes.'); }
            const data = await response.json(); setResults(data.results);
    ...
    const importRecipe = async (recipeId) => {
        ...
        try {
            const response = await fetch(`https://api.spoonacular.com/recipes/${recipeId}/information?includeNutrition=true`, { headers: { 'x-api-key': SPOONACULAR_API_KEY } });
            ...
            const newRecipe = {
                ...
                // UPDATED: Save as an object array
                ingredients: data.extendedIngredients?.map(ing => ({
                    original: ing.original,
                    cleanName: ing.nameClean || cleanIngredient(ing.name) // Use nameClean or our fallback
                })) || [],
    ...
    // ... (rest of ImportRecipes JSX is the same) ...
    return ( <div> <h2 className="text-2xl font-bold text-gray-700 dark:text-gray-200 mb-4">Import from Spoonacular</h2> <form onSubmit={searchRecipes} className="flex gap-4 mb-8"> <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="e.g., pasta, chicken curry..." className="flex-grow p-3 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200" /> <button type="submit" disabled={loading} className="flex items-center justify-center gap-2 bg-blue-600 text-white p-3 rounded-lg font-semibold hover:bg-blue-700 transition disabled:bg-gray-400"> <Search /> <span>Search</span> </button> </form> {loading && <div className="flex justify-center items-center p-8"><Loader className="animate-spin text-green-600" size={48} /></div>} {error && <p className="text-red-500 text-center">{error}</p>} <div className="space-y-4"> {results.map(recipe => ( <div key={recipe.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700 rounded-lg"> <div className="flex items-center gap-4"> <img src={recipe.image} alt={recipe.title} className="w-16 h-16 object-cover rounded-md" /> <p className="font-semibold text-gray-800 dark:text-gray-100">{recipe.title}</p> </div> <button onClick={() => importRecipe(recipe.id)} disabled={importingId === recipe.id} className="flex items-center gap-2 bg-green-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-green-700 transition disabled:bg-gray-400"> {importingId === recipe.id ? <Loader className="animate-spin"/> : <DownloadCloud />} <span>{importingId === recipe.id ? 'Importing...' : 'Import'}</span> </button> </div> ))} </div> </div> );
};

// --- Manage Recipes Component (No changes needed, just ensure modals are defined) ---
const ManageRecipes = ({ refetchTrigger }) => {
    const [recipes, setRecipes] = useState([]); const [loading, setLoading] = useState(true); const [selectedRecipe, setSelectedRecipe] = useState(null); const [editingRecipe, setEditingRecipe] = useState(null);
    const fetchRecipes = async () => { setLoading(true); const querySnapshot = await getDocs(collection(db, "recipes")); setRecipes(querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }))); setLoading(false); };
    useEffect(() => { fetchRecipes(); }, [refetchTrigger]);
    const handleDelete = async (e, id) => { e.stopPropagation(); if (window.confirm('Are you sure?')) { await deleteDoc(doc(db, "recipes", id)); fetchRecipes(); } };
    ...
    return ( <div> <h2 className="text-2xl font-bold text-gray-700 dark:text-gray-200 mb-4">Manage Recipes ({recipes.length})</h2> <div className="overflow-x-auto"> <table className="w-full text-left"> <thead className="bg-gray-50 dark:bg-gray-700 text-sm font-semibold text-gray-600 dark:text-gray-300"> <tr><th className="p-4">Title</th><th className="p-4">Cuisine</th><th className="p-4">Difficulty</th><th className="p-4 text-center">Actions</th></tr> </thead> <tbody> {recipes.map(recipe => ( <tr key={recipe.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 cursor-pointer" onClick={() => setSelectedRecipe(recipe)}> <td className="p-4 font-semibold text-gray-800 dark:text-gray-100">{recipe.title}</td> <td className="p-4 text-gray-600 dark:text-gray-400">{recipe.cuisine}</td> <td className="p-4 text-gray-600 dark:text-gray-400">{recipe.difficulty}</td> <td className="p-4 flex justify-center items-center gap-2"> <button onClick={(e) => { e.stopPropagation(); setEditingRecipe(recipe); }} className="text-yellow-500 hover:text-yellow-700 p-2 rounded-full hover:bg-yellow-100 dark:hover:bg-gray-600"><Pencil size={20} /></button> <button onClick={(e) => handleDelete(e, recipe.id)} className="text-red-500 hover:text-red-700 p-2 rounded-full hover:bg-red-100 dark:hover:bg-gray-600"><Trash2 size={20} /></button> </td> </tr> ))} </tbody> </table> </div> {selectedRecipe && <RecipeDetailModal recipe={selectedRecipe} onClose={() => setSelectedRecipe(null)} />} {editingRecipe && <EditRecipeModal recipe={editingRecipe} onClose={() => setEditingRecipe(null)} onUpdate={handleUpdate} />} </div> );
};
// --- Manage Users Component (No changes needed) ---
const ManageUsers = () => {
...
const AddRecipeForm = ({ onAddSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // UPDATED: Convert manual ingredients to new object structure
            const ingredientsArray = recipe.ingredients.split(',').map(item => ({
                original: item.trim(),
                cleanName: cleanIngredient(item.trim()) // Use our helper
            }));
            ...
            await addDoc(collection(db, 'recipes'), {
                ...recipe,
                ingredients: ingredientsArray, // Save new structure
...
// UPDATED: EditRecipeModal to handle new ingredient structure
const EditRecipeModal = ({ recipe, onClose, onUpdate }) => {
    // Handle both old (string array) and new (object array) data structures
    const getIngredientString = (ingredients) => {
        if (!ingredients || ingredients.length === 0) return '';
        if (typeof ingredients[0] === 'object' && ingredients[0].original) {
            return ingredients.map(ing => ing.original).join(', ');
        }
        return ingredients.join(', ');
    };

    const [formData, setFormData] = useState({ ...recipe, ingredients: getIngredientString(recipe.ingredients), instructions: Array.isArray(recipe.instructions) ? recipe.instructions.join('\n') : '', tags: Array.isArray(recipe.tags) ? recipe.tags.join(', ') : '' });
    const [loading, setLoading] = useState(false);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Convert strings back to new object structure
            const updatedIngredients = formData.ingredients.split(',').map(item => ({
                original: item.trim(),
                cleanName: cleanIngredient(item.trim())
            }));
            ...
            // If nutrition object exists, make sure not to overwrite it
            if (recipe.nutrition) {
                updatedData.nutrition = recipe.nutrition;
            }

            await updateDoc(recipeRef, updatedData);
            onUpdate({ ...updatedData, id: recipe.id });
            onClose();
        } catch (error) { console.error("Error updating document: ", error); alert("Failed to update recipe."); } finally { setLoading(false); }
    };
    
    // ... (rest of EditRecipeModal JSX is the same) ...
